...
/* File: core_main.c
   ...
   initial parameters, run the benchmark and report the results.
*/
#include "coremark.h"
...
__uint32_t time_ms_ticks = 0;

void timer1_isr_init(unsigned int frequency, isr_t handler)
{
    timer1_en_write(0);
    timer1_reload_write(75000000 / frequency - 1);
    timer1_load_write(0);
    timer1_ev_pending_write(timer1_ev_pending_read());
    timer1_ev_enable_write(1);
    if (irq_attach)
        irq_attach(TIMER1_INTERRUPT, handler);
...
main(int argc, char *argv[])
{
  ...
  irq_setmask(0);
	...
	uart_init();
  timer1_isr_init(1000,(isr_t)time1_handler);
